...
def eval_group(pred, label):
    def safe_colname(col_unit):
        # Handle tuple or string
        if isinstance(col_unit, tuple):
            col = col_unit[1]
        else:
            col = col_unit
        return col.split(".")[1] if isinstance(col, str) and "." in col else col

    pred_cols = [safe_colname(unit) for unit in pred['groupBy']]
    label_cols = [safe_colname(unit) for unit in label['groupBy']]
...
def evaluate(gold, predict, db_dir, etype, kmaps):
    ...
    for p, g in zip(plist, glist):
        ...
        if etype in ["all", "match"]:
            ...
            print("=" * 80)
            print(f"[{hardness.upper()}]")
            print(f"🔸 GOLD SQL: {g_str}")
            print(f"🔹 PRED SQL: {p_str}")
            print(f"✅ Exact Match: {exact_score}")
            print("📊 Partial F1 Scores:")

            for k in partial_types:
                v = partial_scores.get(k)
                if v and isinstance(v, dict) and 'f1' in v:
                    print(f"  - {k:<18}: F1 = {v['f1']:.2f}")
                else:
                    print(f"  - {k:<18}: F1 = n/a")
...
def rebuild_col_unit_col(valid_col_units, col_unit, kmap):
    ...
    # FIX: Safely unpack 2-tuple or 3-tuple
    if len(col_unit) == 3:
        ...
        raise ValueError(f"Unexpected col_unit format: {col_unit}")
...
def build_foreign_key_map(entry):
    foreign_key_map = {}
    for key1, key2 in entry["foreign_keys"]:
        foreign_key_map[key1] = key2
        foreign_key_map[key2] = key1
...
def build_foreign_key_map_from_json(table):
    ...
    tables = {}
    for entry in data:
        entry["foreign_keys"] = [ (tuple(fk[0]), tuple(fk[1])) for fk in entry["foreign_keys"] ]
