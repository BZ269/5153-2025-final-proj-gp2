...
def upload_to_weaviate(chunks, index_name, client, llm=None):
    ...
    if llm is None:
        ...
        try:
            llm = OllamaEmbeddings(model="llama3.1")
...
if __name__ == "__main__":
    ...
    ### Initialize ChatOllama model and embeddings
    chat_model = ChatOllama(model="llama3.1")
    llm = OllamaEmbeddings(model="qwen2.5:14b-instruct")
    
    ### Process the posters for full poster chunks
    full_poster_chunks = process_posters_as_whole_chunks(chat_model=chat_model)
    # print(full_poster_chunks)

    ### Upload to Weaviate if chunks exist
    if full_poster_chunks:
        upload_to_weaviate(full_poster_chunks, "MedicalQIDocument_Poster_Full", client, llm)
    else:
        print("❌ No poster chunks generated. Please check your document processing.")